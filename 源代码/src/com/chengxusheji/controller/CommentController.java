...
public class CommentController extends BaseController {
	...
	public void list(@ModelAttribute("minsuObj") Minsu minsuObj,@ModelAttribute("userObj") UserInfo userObj,String commentTime,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
	...
	public String frontlist(@ModelAttribute("minsuObj") Minsu minsuObj,@ModelAttribute("userObj") UserInfo userObj,String commentTime,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (commentTime == null) commentTime = "";
		List<Comment> commentList = commentService.queryComment(minsuObj, userObj, commentTime, currentPage);
	    /*计算总的页数和总的记录数*/
	    commentService.queryTotalPageAndRecordNumber(minsuObj, userObj, commentTime);
	    /*获取到总的页码数目*/
	    int totalPage = commentService.getTotalPage();
	...
	public String userFrontlist(@ModelAttribute("minsuObj") Minsu minsuObj,@ModelAttribute("userObj") UserInfo userObj,String commentTime,Integer currentPage, Model model, HttpServletRequest request,HttpSession session) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (commentTime == null) commentTime = "";
	...
	public void OutToExcel(@ModelAttribute("minsuObj") Minsu minsuObj,@ModelAttribute("userObj") UserInfo userObj,String commentTime, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(commentTime == null) commentTime = "";
        List<Comment> commentList = commentService.queryComment(minsuObj,userObj,commentTime);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Comment信息记录"; 
        String[] headers = { "评论id","被评茂名旅游","评论内容","评论用户","评论时间"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<commentList.size();i++) {
        	Comment comment = commentList.get(i); 
        	dataset.add(new String[]{comment.getCommentId() + "",comment.getMinsuObj().getMinsuName(),comment.getContent(),comment.getUserObj().getName(),comment.getCommentTime()});
        }
        /*
