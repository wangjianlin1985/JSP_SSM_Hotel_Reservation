	...
	$.ajax({
		...
		success : function (comment, response, status) {
			...
			if (comment) { 
				...
				$("#comment_minsuObj_minsuId_edit").combobox({
					url:"Minsu/listAll",
					valueField:"minsuId",
					textField:"minsuName",
					panelHeight: "auto",
					...
					onLoadSuccess: function () { //数据加载完毕事件
						$("#comment_minsuObj_minsuId_edit").combobox("select", comment.minsuObjPri);
						//var data = $("#comment_minsuObj_minsuId_edit").combobox("getData"); 
						//if (data.length > 0) {
							//$("#comment_minsuObj_minsuId_edit").combobox("select", data[0].minsuId);
						//}
					}
				});
